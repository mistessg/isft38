    ...
    <form class="container ">
        <div class="card">
            ...
            <div class=" mb-3">
                ...
                <input type="nombre" name="nombre" class="form-control" placeholder="Escriba su nombre" id="nombre" aria-describedby="emailHelp" required>
            </div>

        <div class="mb-3">
            <label for="email" class="form-label">Email</label>
            <input type="email" name="email" class="form-control" id="email" placeholder="Escriba su email" aria-describedby="emailHelp" required>
        </div>

        <div class="mb-3">
            <label for="telefono" class="form-label">Teléfono</label>
            <input type="number" name="telefono" class="form-control" id="telefono" placeholder="Escriba su teléfono" aria-describedby="emailHelp" required>
        </div>

        <div class="mb-3" >
            <label for="sede" class="form-label">Sede <i style="font-weight:normal;"></i></label>
            <select name="sede" style="cursor:pointer;" class="form-select" name="sede" id="sede" placeholder="Elija la sede" required>
        ...
        <div class="mb-3">
            ...
            {{ Form::textarea("objetivo", old("objetivo"), ["class" => "form-control", "name"=>"objetivo", "id"=>"summernote", "placeholder" => "Ingrese su mensaje" ]) }}
          @error('objetivo')
              <div class="alert alert-danger">{{ $message }}</div>
          @enderror


        <div class="boton"><button type="submit"  class="btn mt-4">Enviar</button></div>
...
<div class="my-4 container-map_icons">
    ...
    @foreach($sedes as $sede)
        ...
        <div style="display:flex;align-items:center;">
            ...
            @if($loop->first)
            <p class="pt-3 icon-text" style="font-weight:bold;">{{$telef->caracteristica}} - {{$telef->numero}} </p>
            @else
            <p class="pt-3 icon-text" style="font-weight:bold;">{{$telef->caracteristica}} - {{$telef->numero}}</p> 
            @endif
            @endforeach
